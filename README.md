...
```json
{
  "server": "JWRTPMS",
  "tag": "JW_waterlevelmeter",
  "starttime": "2026-01-01 11:55",
  "endtime": "2026-01-02 11:55"
}
```
...
```text
GET /api/waterlevel/history
  ?start=2026-01-01%2011:55
  &end=2026-01-02%2011:55
```

規則：

- `start` / `end` 使用 `YYYY-MM-DD HH:mm`
- 時區固定 Asia/Taipei
- 結束時間必須晚於開始時間
- 單次最多查 7 天
- server / tag 固定在後端，前端不能任意修改
- PI 高頻原始資料先轉成 1 分鐘中位數
- 若圖表點太多，後端再自動降採樣，避免瀏覽器卡頓

### 歷史降雨

水位歷史可直接向 PI Recorded API 查指定日期。

QPE 歷史是本站自行保存，因此只會疊加「本站實際保存到」的日期，不會用其他日期資料補值。
v10.2 將 QPE history 保留時間由 72 小時延長為：

```text
168 小時 = 7 天
```

所以部署滿 7 天後，在最近 7 天範圍內查詢時可以同時看到：

```text
台塑仁四橋水位折線
+
仁武 10 km QPE 過去1h面平均
```

若查詢早於本站 QPE 保存期間，例如很久以前的日期：
...
首頁標題改為：`仁武廠區即時防災雨量儀表板`。


## v10.3：統一水位圖尺度

這版只調整水位趨勢圖的尺度與取樣方式，不改：
...
以前每張圖會依該時間區間的最低 / 最高水位自動縮放，因此短區間只有幾公分變化時，看起來也可能非常劇烈。
...
不再把 0.06 m 填滿整個圖，而會保留約 0.10 m 以上的視覺範圍。
...
後端 `/api/waterlevel/history` 也採同樣規則，不再依原始資料筆數動態算「約 1500 點」。
...
這個差異是刻意保留，避免跨日圖無法辨認日期。


## v10.3.1：QPE 右側 Y 軸標籤修正

v10.3 的水位折線與 QPE 柱狀資料本身正確，但右側 QPE Y 軸文字顛倒：
...
本版只修正右側刻度標籤換算，不修改水位資料、QPE 柱狀資料、
5/10/15 分鐘取樣、警戒值、日期查詢或其他模型資料。
