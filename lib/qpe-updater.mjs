...
async function appendQpeHistory(pathname, latest, retentionHours = 168) {
  const time = latest?.observation_time;
  ...
  const row = {
    time,
    point_1h_mm: Number.isFinite(Number(latest?.past_1h_qpe_mm))
      ? Number(latest.past_1h_qpe_mm)
      : null,
    radius_5km_mean_1h_mm: Number.isFinite(Number(latest?.area?.radius_5km?.mean_mm))
      ? Number(latest.area.radius_5km.mean_mm)
      : null,
    radius_10km_mean_1h_mm: Number.isFinite(Number(latest?.area?.radius_10km?.mean_mm))
      ? Number(latest.area.radius_10km.mean_mm)
      : null,
    radius_10km_max_1h_mm: Number.isFinite(Number(latest?.area?.radius_10km?.max_mm))
      ? Number(latest.area.radius_10km.max_mm)
      : null,
    radius_10km_ge20_pct: Number.isFinite(Number(latest?.area?.radius_10km?.coverage_pct?.ge_20))
      ? Number(latest.area.radius_10km.coverage_pct.ge_20)
      : null
  };

  const byTime = new Map(series.map(x => [x?.time, x]));
  byTime.set(time, row);

  const cutoff = Date.now() - retentionHours * 60 * 60 * 1000;
  const kept = [...byTime.values()]
    .filter(x => {
  ...
  await writeAtomic(pathname, {
    status: "ok",
    schema_version: 1,
    updated_at: isoNow(),
    retention_hours: retentionHours,
    meaning:
      "Each QPE value is the rolling past-1h estimate at that observation time; do not sum adjacent rows.",
    series: kept
  });
}
...
export function createQpeUpdater({
  ...
  const historyPath = path.join(dataDir, "qpe-history-renwu.json");
  let timer = null;
  let running = false;
  ...
  async function checkAndUpdate({ reason = "interval" } = {}) {
    ...
    try {
      ...
      // Keep a small rolling history for the water-level + rainfall chart.
      // Values are rolling past-1h QPE snapshots, not additive 10-minute rain.
      await appendQpeHistory(historyPath, latest, 168);
