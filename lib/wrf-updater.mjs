...
export function createWrfUpdater({
  ...
  async function checkAndUpdate({
    ...
    try {
      ...
      const hasAreaSchema =
        Number(current?.schema_version || 0) >= 3 &&
        current?.forecast?.some(x => x?.area?.radius_10km);
      ...
      if (
        ...
        status.last_action =
          `無新批次：${meta.initialTime}，沿用現有 WRF 面積雨量 JSON`;
        return {
          ok: true,
          updated: false,
      ...
      status.last_action =
        ...
        localInitial === meta.initialTime &&
        !hasAreaSchema
          ? `WRF ${meta.initialTime} 舊 JSON 缺少面積統計，執行一次 v8 資料升級`
